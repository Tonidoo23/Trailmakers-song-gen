...
const stepDelay = (bpm: number) => Number((60 / bpm / 2).toFixed(2));
...
function App() {
  ...
  const generate = () => {
    ...
    for (let i = 0; i < leadCount; i++) {
      ...
      const semi = mod(tones[toneIdx % 3] + octShift);
      generated.push({
        id: `lead-${i + 1}`,
        label: `L${i + 1}`,
        type: 'lead',
        note: SEMI_TO_NOTE[semi],
        neigung: tones[toneIdx % 3] + octShift,
        delay: Number((i * d).toFixed(2)),
        duration: Number((0.2 * durMult).toFixed(2)),
        pause: 0,
        sound: leadSound,
        trigger: 'Ein-/Ausschalten (StructureRight)',
        toggle: false,
        x: i % 8,
        y: Math.floor(i / 8),
      });
    ...
    if (includeBass) {
      chords.forEach((ch, i) => {
        const rootSemi = NOTE_TO_SEMITONE[ch.root] - 12 + octShift;
        generated.push({
          id: `bass-${i + 1}`,
          label: `B${i + 1}`,
          type: 'bass',
          note: SEMI_TO_NOTE[mod(rootSemi)],
          neigung: rootSemi,
    ...
    generated.push({
      id: 'seat-1', label: 'Seat', type: 'seat', x: 0,
      y: Math.max(...generated.map((g) => g.y)) + 1,
      trigger: 'Ein-/Ausschalten (StructureRight)', toggle: false,
    });
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <section className="rounded bg-slate-900 p-4">
          ...
          <div className="grid grid-cols-8 gap-2">
            {blocks.map((b) => (
              <button key={b.id} onClick={() => setSelected(b)} className={`rounded p-2 text-xs ${selected?.id === b.id ? 'bg-cyan-700' : 'bg-slate-800'}`}>
                {b.label}<br />({b.x},{b.y})
              </button>
            ))}
          </div>
